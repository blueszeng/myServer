...
func handleRegister(args []interface{}) {
	...
	log.Debug("RRegister AccID:%v PassWD:%v Sex:%v", m.AccID, m.PassWD, m.Sex)

	if len(m.AccID) < gamedata.AccIDMin || len(m.AccID) > gamedata.AccIDMax {
		log.Debug("账号不合法 AccID:%v", m.AccID)
		a.WriteMsg(&msg.S2C_Register_Ret{Ret: msg.S2C_Register_AccError})
		return
	}
...
func handleLogin(args []interface{}) {
	...
	if len(m.AccID) < gamedata.AccIDMin || len(m.AccID) > gamedata.AccIDMax {
		a.WriteMsg(&msg.S2C_Login_Ret{Ret: msg.S2C_Login_AccIDInvalid})
		return
	}
